...
void
BriteTopologyHelper::AssociateNodeInfo()
{
    NS_LOG_FUNCTION(this);

    for (uint32_t i = 0; i < m_briteNodeInfoList.size(); ++i)
    {
        auto node = m_nodes.Get(i);
        auto nodeInfo = &m_briteNodeInfoList.at(i);
        m_briteNodeInfoMap.insert({node->GetId(), nodeInfo});
    }
}

Vector2D
BriteTopologyHelper::GetNodePosition(uint32_t nodeId)
{
    NS_LOG_FUNCTION(nodeId);
    auto info = m_briteNodeInfoMap.at(nodeId);
    return Vector2D(info->xCoordinate, info->yCoordinate);
}
